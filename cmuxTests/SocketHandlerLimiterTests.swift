...
/// lifetime hazards of the bounded socket-handler admission in
/// `TerminalController.spawnClientHandler` (a leaked permit silently shrinks
/// the cap; an over-released permit inflates it). Covered:
///
...
final class SocketHandlerLimiterTests: XCTestCase {
    ...
    /// Models the exact owner-drop hazard from
    /// `TerminalController.spawnClientHandler`:
    ///
