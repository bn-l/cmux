    ...
    static func parse(tokens: [String]) -> Result<RightSidebarRemoteRequest, RightSidebarRemoteParseError> {
        ...
        case "set":
            guard positional.count == 2 else {
                return .failure(.init(message: String(localized: "rightSidebar.remote.error.usage.set", defaultValue: "ERROR: Usage: right_sidebar set <files|find|feed|dock> [--no-focus] [--workspace=<workspace-id>] [--window=<window-id>]")))
            }
