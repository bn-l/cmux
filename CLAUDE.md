...
- **Never build a spawned test process's environment from `ProcessInfo.processInfo.environment`.** Doing so inherits the real environment of whoever launched the test runner. When that is a cmux terminal (the normal case — `reload.sh`, or Xcode opened from a cmux tab) it carries `CMUX_CODEX_WRAPPER_SHIM` / `CMUX_CLAUDE_WRAPPER_SHIM`, and the resume command cmux generates (`AgentResumeArgv.swift`) prefers that shim over a `PATH` lookup. The fake agent stub the test installed is bypassed and the user's **real, authenticated agent** is launched — the observed case was `codex resume <fake-session> --yolo`, orphaned to launchd, spinning CPU for 46 minutes with Codex closed. Prepending a fake bin to `PATH` does **not** save you: `zsh -lc` is a login shell, so `path_helper` rebuilds `PATH` from `/etc/paths.d` (which includes `/opt/homebrew/bin`) ahead of your entry — verify with `env -i HOME=<empty> PATH=<fakebin>:/usr/bin:/bin zsh -lc 'command -v codex'`. Use `AgentSpawnIsolation.childEnvironment(home:prependingPath:fakeAgents:)` in `cmuxTests/AgentSpawnIsolationTestSupport.swift`, which binds the shim variable to your stub so resolution never touches `PATH`. `Resources/bin/cmux-{codex,claude}-wrapper` refuse to exec under XCTest as a safety net, and `scripts/test-unit.sh` brackets every run with `scripts/reap-leaked-agents.sh`; run `just agents-leaked` if you suspect a stray. Treat both as backstops, not permission to skip the helper.
- **SPM packages live in group folders, and the root workspace mirrors that folder shape exactly.** Every Swift package lives physically under exactly one group directory — `Packages/Shared/<pkg>` (shared across cmux targets) or `Packages/macOS/<pkg>` (macOS app only) — and `cmux.xcworkspace/contents.xcworkspacedata` has groups whose container locations are those folders, with every package directory appearing as a FileRef under its folder's group. So opening the workspace shows all packages grouped exactly like the directory tree. The folder is the source of truth: to move a package between groups, `git mv` its directory, then run `python3 scripts/check-workspace-package-groups.py --write` to regenerate the workspace. A new package goes in the group folder matching its consumers. Cross-group `.package(path:)` deps use `../../<Group>/<Name>`; never hand-edit the workspace group membership. CI's `python3 scripts/check-workspace-package-groups.py --check` fails on drift.
